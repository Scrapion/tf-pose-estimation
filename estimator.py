...
class Human:
    ...
    """
    __slots__ = ('body_parts', 'pairs', 'uidx_list', 'score','x_parts','y_parts','scores')
...
class TfPoseEstimator:
    ...
    #print human having coordinates array in input
    @staticmethod
    def draw_human(npimg, lhuman, imgcopy=False):
        human = Human([])
        j=0
        part_idx = 0
        human_id  = 0
        while j < len(lhuman):
        #RICCARDO //TODO 
            if(lhuman[j] != lhuman[j+1]):
                human.x_parts.append(lhuman[j])
                human.y_parts.append(lhuman[j+1])
                
                human.body_parts[part_idx] = BodyPart(
                    '%d-%d' % (human_id, part_idx), part_idx,
                    float((lhuman[j]/2)/(npimg.shape[1]/2)),
                    float((lhuman[j+1]/2)/(npimg.shape[0]/2)),
                    1
                )
            j+=2
            part_idx+=1
